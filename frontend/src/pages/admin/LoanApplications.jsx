...
const LoanApplications = () => {
  ...
  return (
    <DashboardLayout isAdmin>
      <h1 style={{ fontSize: 26, marginBottom: 4 }}>Loan applications</h1>
      <p style={{ color: 'var(--color-text-muted)', marginBottom: 20 }}>Review, approve, or reject customer applications.</p>
      ...
      {loading ? <Spinner /> : (
        <div style={{ display: 'grid', gap: 12 }}>
          {loans.length === 0 && <div className="card" style={{ textAlign: 'center', padding: 40, color: 'var(--color-text-muted)' }}>No applications in this category.</div>}
          {loans.map((l) => (
            <div key={l._id} className="card" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ flex: 1 }}>
